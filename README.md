...
### 6. `CLAUDE.md` (via `--format claude`)

Agent-optimized context file for [Claude Code](https://docs.anthropic.com/en/docs/claude-code). Contains project overview, directory structure, architecture patterns, tech stack, commands, conventions, and deployment info — structured to help AI coding agents understand your codebase quickly.
...
The `examples/` directory contains sample reports generated by running `conventions discover` on the [FastAPI](https://github.com/fastapi/fastapi) open-source repository:
