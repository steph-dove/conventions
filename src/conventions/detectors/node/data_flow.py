...
_NODE_EXTENSIONS = (".ts", ".tsx", ".js", ".jsx", ".mjs", ".cjs")
_INDEX_FILES = ("index.ts", "index.js", "index.tsx", "index.jsx")
...
def _resolve_import(
    ...
    """Resolve a Node.js import to a relative file path in the index.

    Args:
        importing_file: Relative path of the importing file.
        module_spec: The import string (e.g., './services/user', 'express').
    ...
    if module_spec.startswith("@/"):
        # Alias: resolve against src/
        base_path = module_spec.replace("@/", "src/", 1)
    else:
        # Relative import: resolve against importing file's directory
        importing_dir = str(Path(importing_file).parent)
        base_path = os.path.normpath(os.path.join(importing_dir, module_spec))

    # Normalize path separators
    base_path = base_path.replace("\\", "/")

    # Try exact match first
    if base_path in known_files:
        return base_path

    # Try with extensions
    for ext in _NODE_EXTENSIONS:
...
class NodeDataFlowDetector(NodeDetector):
    """Detect Node.js data flow patterns."""

    name = "node_data_flow"
    description = "Detects Node.js data flow: import graph, endpoint chains, service dependencies"
    ...
    def _build_graph_from_index(
        ...
        """Build graph data structures from NodeIndex."""
        ...
        for rel_path, file_idx in index.files.items():
            ...
            for module_spec, line in file_idx.imports:
                resolved = _resolve_import(rel_path, module_spec, known_files)
    ...
    def _emit_import_graph_rule(
        ...
        result.rules.append(self.make_rule(
            rule_id="node.data_flow.import_graph",
            category="data_flow",
    ...
    def _emit_endpoint_chains_rule(
        ...
        # Evidence: first chain's endpoint file
        evidence = []
        ...
        result.rules.append(self.make_rule(
            rule_id="node.data_flow.endpoint_chains",
            category="data_flow",
    ...
    def _emit_service_deps_rule(
        ...
        # Map each service file to its store/db dependencies
        ...
        result.rules.append(self.make_rule(
            rule_id="node.data_flow.service_dependencies",
            category="data_flow",
